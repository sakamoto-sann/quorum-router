...
export async function runAuthStatus(): Promise<void> {
  ...
  console.log("Fusion Router auth status");
...
export async function runAuthLogout(): Promise<void> {
  try {
    await Deno.remove(".fusion-router", { recursive: true });
  } catch (error) {
    if (!(error instanceof Deno.errors.NotFound)) throw error;
  }
  console.log("Fusion Router auth logout");
  console.log(
    "Removed generated local session metadata directory if present: .fusion-router/",
  );
  console.log(
