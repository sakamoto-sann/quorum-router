...
function githubHeaders(): HeadersInit {
  const headers: Record<string, string> = {
    ...
    "user-agent": "fusion-router-generated-dogfood",
  };
...
function pathPriority(path: string): number {
  ...
  if (/^packages\/create-fusion-router\/templates\/basic\/src\//.test(lower)) {
    return 2;
  }
  if (
  ...
  if (/^(packages\/create-fusion-router|\.github\/workflows)\//.test(lower)) {
    return 4;
  }
